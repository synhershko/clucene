...
#define _CLDECDELETE(x) if (x!=NULL){ CND_PRECONDITION((x)->__cl_refcount>=0,"__cl_refcount was < 0"); _LUCENE_ATOMIC_DECDELETE(CL_NS(debug)::LuceneBase*, x->THIS_LOCK, &(x)->__cl_refcount, x); x=NULL; }
#define _CLLDECDELETE(x) if (x!=NULL){ CND_PRECONDITION((x)->__cl_refcount>=0,"__cl_refcount was < 0"); _LUCENE_ATOMIC_DECDELETE(CL_NS(debug)::LuceneBase*, x->THIS_LOCK, &(x)->__cl_refcount, x); }
